...
(function () {
    ...
    ko.validation = {
        ...
        group: function (obj) { // array of observables or viewModel
            ...
            result.showAllMessages = function () {
                ...
                var firstErrorEl = $('span.error:visible:first');                
                if(firstErrorEl && firstErrorEl[0]) {
                    $('#maincontainer').scrollTop(firstErrorEl.offset().top);
                    firstErrorEl.prev() && firstErrorEl.prev().focus && firstErrorEl.prev().focus();
                }
            };
            result.hideAllMessages = function () {
                ko.utils.arrayForEach(observables, function (observable) {
        ...
        insertValidationMessage: function (element) {
            ...
            var errorClass = (prev && prev[0]) ? 'error error-vertical' : 'error';
            var el = $('<span class="'+errorClass+'" style=""><label generated="true" class="error" style="display: inline; "><span class="text">text</span><span class="tip"></span></label></span><span class="icon tick valid"></span>').insertAfter(element);
            return el[0];
        },
        ...
        registerValueBindingHandler: function () { // parse html5 input validation attributes where value binder, optional feature
            ...
            ko.bindingHandlers.value.init = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext, isInitiated) {

                // edited by vrubalka: added isInitiated property, prevent 2 validation messages
                init(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext, true);
    ...
    ko.bindingHandlers['validationMessage'] = { // individual error message, if modified or post binding
        update: function (element, valueAccessor) {
            ...
            var textEl = $(element).find('span.text');
            if(textEl && textEl[0]) {
                ko.bindingHandlers.text.update(textEl[0], errorMsgAccessor);
            }
        }
    };
    ...
    ko.validation.rules['email'] = {
        validator: function (val, validate) {
            return validate && (/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i.test(val) || !val || val === '');
        },
